...
class SyncOllamaClient:
    """A dedicated synchronous client for the background CompactionEngine[cite: 24]."""

    def __init__(
        self, model="llama3", url="http://localhost:11434/api/generate"
    ):
        self.model = model
        self.url = url
...
# Pass the actual 'rom_db' variable, not the placeholder[cite: 24]
compaction_engine = CompactionEngine(
    rom_connection=rom_db, llm_client=sync_llm
)
...
class ChatPayload(BaseModel):
    message: str
    session_id: str  # Now explicitly required to track conversations[cite: 24]
    user_auth: Optional[str] = None


def get_or_create_session(session_id: str) -> RAMWindow:
    """Retrieves an active RAM window, or builds one from ROM if it's missing[cite: 24]."""
    if session_id not in active_sessions:
        ...
        for msg in past_context:
            # Now passing row_id dynamically from the database[cite: 24]
            new_window.add_interaction(
                role=msg["role"], content=msg["content"], row_id=msg["id"]
            )
...
async def talk_to_v(payload: ChatPayload, bg_tasks: BackgroundTasks):
    if not payload.message or not payload.session_id:
        raise HTTPException(
            status_code=400,
            detail="Payload requires 'message' and 'session_id'.",
        )

    try:
        ram = get_or_create_session(payload.session_id)

        # 1. Save user message to ROM & RAM[cite: 24]
        user_row_id = rom_db.save_message(
            payload.session_id, "user", payload.message
        )
        ram.add_interaction("user", payload.message, row_id=user_row_id)

        # 2. Run the graph (same as before)[cite: 24]
        ...
        # 3. Save V's response to ROM & RAM[cite: 24]
        assistant_row_id = rom_db.save_message(
            payload.session_id, "assistant", final_response
        )
        ram.add_interaction(
            "assistant", final_response, row_id=assistant_row_id
        )

        # 4. Trigger Background Compaction if Critical[cite: 24]
        if ram.is_critical:
            # Instantly remove the oldest 4 messages from active RAM[cite: 24]
            stale_messages = ram.extract_for_compaction()
            # Send them to the background queue so the API can return immediately[cite: 24]
            bg_tasks.add_task(
                compaction_engine.compact_messages, stale_messages
            )
            print(
                f"[SYSTEM] Offloaded {len(stale_messages)} messages to background compaction."
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"SYSTEM_CRASH: {str(e)}"
        )