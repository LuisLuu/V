...
async def check_auth_status():
    """Checks if a Master PIN has been set up in the database[cite: 23]."""
...
async def setup_pin(payload: PINPayload):
    """Sets the Master PIN for the first time[cite: 23]."""
    if len(payload.pin) < 4:
        raise HTTPException(
            status_code=400, detail="PIN must be at least 4 characters."
        )
...
async def verify_pin(payload: PINPayload):
    """Verifies the provided PIN to unlock the UI[cite: 23]."""
