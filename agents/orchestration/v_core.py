...
from prompts.core_prompts import ORCHESTRATOR_PROMPT, V_PERSONA
...
class VCore:
    ...
    async def planner_llm_call(
        ...
        system_instruction = (
            ...
            f"[SYSTEM ROM: USER PREFERENCES & CONTEXT]:\n{user_context}\n\n"
            f"[SYSTEM ROM: AUTONOMOUS MEMORY BANK]:\n{learned_facts}\n\n..."
        )
        ...
        try:
            async with aiohttp.ClientSession(timeout=ENGINE_TIMEOUT) as session:
                print(
                    "[DEBUG] aiohttp session opened. Sending POST to Ollama..."
                )
                payload = {
                    "model": MODEL_NAME,
                    "messages": messages,
                ...
                async with session.post(
                    ...
                    print(
                        f"[DEBUG] Ollama responded with status code: {response.status}"
                    )
                    if response.status != 200:
                        raise Exception(
                            f"Ollama Planner Error: {response.status}"
                        )
            ...
            raise Exception(
                "Local LLM engine connection timed out. Is Ollama running?"
            )
        except Exception as e:
    ...
    @staticmethod
    async def synthesizer_stream(
        prompt: str, results: list, chat_history: List[Dict[str, str]]
