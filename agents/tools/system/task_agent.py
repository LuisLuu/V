...
class TaskAgent:
    ...
    Bypasses RAM and Compaction layers completely[cite: 12].
    """
    ...
    def create_task(
        ...
        """Writes a new task to ROM, now supporting deadlines[cite: 12]."""
        try:
    ...
    def list_tasks(
        ...
        """Queries the active tasks ledger, enforcing priority ordering[cite: 12]."""
    ...
    def update_task(
        ...
        """Modifies a task's state safely using either exact ID or constrained title match[cite: 12]."""
        valid_statuses = ["pending", "in_progress", "completed", "cancelled"]
        if status not in valid_statuses:
            return {
                "status": "error",
                "message": f"Invalid state. Must be one of {valid_statuses}.",
            }

        if not task_id and not title:
            return {
                "status": "error",
                "message": "Must provide either task_id or title to update.",
            }
        ...
        try:
            ...
            return {
                "status": "success",
                "message": f"[SYSTEM NOTIFICATION]: Task ID {target_id} successfully marked as {status}. Inform the user.",
            }
        except Exception as e:
    ...
    def delete_task(
        self, task_id: Optional[int] = None, title: Optional[str] = None
    ) -> Dict[str, Any]:
        """Permanently removes a single task safely using either exact ID or single title match[cite: 12]."""
        if not task_id and not title:
            return {
                "status": "error",
                "message": "Must provide either task_id or title to delete.",
            }
        ...
        try:
            ...
            if affected == 0:
                return {
                    "status": "error",
                    "message": f"Task {target_id} missing or already deleted.",
                }

            logger.info(f"Task {target_id} deleted.")
            return {
                "status": "success",
                "message": f"[SYSTEM NOTIFICATION]: Task ID {target_id} successfully deleted from the ledger. Inform the user.",
            }
