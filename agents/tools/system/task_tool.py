...
class TaskManagerTool(BaseTool):
    ...
    description: str = (
        "Create, read, update, or delete tasks in V's system ledger."
    )
                ...
                "parameters": {
                    ...
                    "properties": {
                        "action": {
                            ...
                            "description": (
                                "The task action to perform. CRITICAL TRANSLATION RULES:\n"
    ...
    def execute(self, action: str, **kwargs) -> Any:
        ...
        if action == "create":
            ...
            if task_id is None and title is None:
                raise ValueError(
                    "Either 'task_id' or 'title' is required to update."
                )

            result = self.agent.update_task(
                status=status, task_id=task_id, title=title
            )
            ...
            if task_id is None and title is None:
                raise ValueError(
                    "Either 'task_id' or 'title' is required for 'delete'."
                )
