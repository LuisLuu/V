...
class BlastGate:
    ...
    Every tool call from the Orchestrator must pass through this gate before
    execution.
    """
    ...
    def evaluate_execution(
        self, tool_name: str, tier: SecurityTier, kwargs: dict
    ) -> dict:
        """Evaluates if the LLM's requested action is safe to execute

        autonomously. Returns a strict dictionary schema for the FastAPI
        router to intercept.
        """
        logging.info(
            f"[BLAST GATE] Evaluating {tool_name} at tier {tier.name}..."
        )
        ...
        if tier == SecurityTier.WRITE:
            if (
                ...
                and str(kwargs.get("method", "")).upper() == "GET"
            ):
                return {
                    "approved": True,
                    "status": "AUTO_APPROVED",
    ...
    def _trigger_hitl(
        ...
        """Formats a structured Human-In-The-Loop (HITL) authorization

        request. The FastAPI router will use this to pause the LLM and ping
        the UI.
        """
        warning_msg = (
        ...
        return {
            ...
            "tool_payload": {"name": tool_name, "kwargs": kwargs},
        }