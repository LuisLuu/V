...
class TaskAgent:
    ...
    def create_task(self, title: str, description: str = "", priority: str = "medium") -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": "success", "task_id": task_id, "message": f"Task '{title}' added to pending."}
    ...
    def list_tasks(self, status_filter: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        # SQLite lacks native enum weighting, so we force-cast the priority order
    ...
    def update_task(self, task_id: int, status: str) -> Dict[str, Any]:
        ...
        try:
            with self.rom._get_connection() as conn:
                cursor = conn.cursor()
                # Explicitly applying CURRENT_TIMESTAMP to overcome the SQLite limitation
                cursor.execute(
